...
function option_prices = analyse_quanto(X0, Sf_0,  Kf, sigX, sigSf, rd, rf, T, Xbar, V)
    ...
    for n = 1:lenN
        ...
        for m = 1:M
            ...
            tic;
            values_crude(m,n)   = quanto_pricer(X0, Sf_0,  Kf, sigX, sigSf, rd, rf, T, Xbar, phi_crude, No_paths);
            times_crude(m,n)    = toc;
        end
    end
    ...
    for n = 1:lenN
        ...
        for m = 1:M
            ...
            tic;
            values_anti(m,n)   = quanto_pricer(X0, Sf_0,  Kf, sigX, sigSf, rd, rf, T, Xbar, phi_anti, No_paths);
            times_anti(m,n)    = toc;
        end
    end    
    % Calculate variances, run times and 95% CL:
    ...
    for n = 1:lenN
        ...
        for m = 1:M
            phi = phi_quasi(:,m);
            values_quasi(m,n)   = quanto_pricer(X0, Sf_0,  Kf, sigX, sigSf, rd, rf, T, Xbar, phi, No_paths);
        end
